...
function registerVariableCompletions(pattern: string): vscode.Disposable {
    return vscode.languages.registerCompletionItemProvider({ language: "jsonc", pattern }, {
        provideCompletionItems(document, position, _token) {
            ...
            if (((document: vscode.TextDocument, location: Location, pos: vscode.Position) => {
                ...
                if (previousNode && previousNode.type === "string") {
                    ...
                    return (offset > previousNode.offset &&
                ...
                return [
                    ...
                    },
                    {
                        ...
                        detail: vscode.l10n.t("The name of the folder opened in VS Code without any slashes (/)"),
                    },
                    {
                        label: "fileWorkspaceFolderBasename",
                        detail: vscode.l10n.t("The current opened file workspace folder name without any slashes (/)"),
                    },
                    {
                        label: "relativeFile",
                    ...
                    },
                    {
                        ...
                        detail: vscode.l10n.t("The current opened file's dirname relative to ${workspaceFolder}"),
                    },
                    {
                        label: "file",
                    ...
                    {
                        label: "pathSeparator",
                        detail: vscode.l10n.t("The character used by the operating system to separate components in file paths. Is also aliased to '/'."),
                    },
                    {
    ...
    return vscode.languages.registerCompletionItemProvider({ pattern: "**/extensions.json" }, {
        provideCompletionItems(document, position, _token) {
            ...
            if (location.path[0] === "recommendations") {
                ...
                return provideInstalledExtensionProposals((extensionsContent &&
                    extensionsContent.recommendations) ||
                    [], "", getReplaceRange(document, location, position), false);
            }
            return [];
        },
    });
...
function getReplaceRange(document: vscode.TextDocument, location: Location, position: vscode.Position) {
    ...
    if (node) {
        ...
        if (nodeStart.isBeforeOrEqual(position) &&
...
function registerContextKeyCompletions(): vscode.Disposable {
    ...
    return vscode.languages.registerCompletionItemProvider([...paths.keys()], {
        async provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken) {
            ...
            if (!isValidLocation ||
                !((document: vscode.TextDocument, location: Location, pos: vscode.Position) => {
                    if (location.isAtPropertyKey) {
                        return false;
                    }
                    const previousNode = location.previousNode;
            ...
            const inserting = (document.getWordRangeAtPosition(position, /[a-zA-Z.]+/) ||
                new vscode.Range(position, position)).with(undefined, position);
