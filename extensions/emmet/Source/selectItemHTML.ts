...
import { findNextWord, findPrevWord, getDeepestFlatNode, getHtmlFlatNode, offsetRangeToSelection, } from "./util";
export function nextItemHTML(document: vscode.TextDocument, selectionStart: vscode.Position, selectionEnd: vscode.Position, rootNode: HtmlNode): vscode.Selection | undefined {
    ...
    if (currentNode.type !== "comment") {
        ...
        if (currentNode.open && selectionEndOffset < currentNode.open.end) {
            const attrSelection = getNextAttribute(document, document.offsetAt(selectionStart), selectionEndOffset, currentNode);
            if (attrSelection) {
...
export function prevItemHTML(document: vscode.TextDocument, selectionStart: vscode.Position, selectionEnd: vscode.Position, rootNode: HtmlNode): vscode.Selection | undefined {
    ...
    if (currentNode.open &&
        ...
        if (selectionStartOffset < currentNode.open.end ||
            ...
            prevNode = <HtmlNode>(getDeepestFlatNode(prevNode && prevNode.type !== "comment"
                ? prevNode
                : oldOption));
        }
    }
...
function getSelectionFromNode(document: vscode.TextDocument, node: HtmlNode): vscode.Selection | undefined {
    if (node && node.open) {
        ...
        return offsetRangeToSelection(document, selectionStart, (selectionStart + node.name.length));
    }
    return undefined;
}
function getNextAttribute(document: vscode.TextDocument, selectionStart: number, selectionEnd: number, node: HtmlNode): vscode.Selection | undefined {
    ...
    for (const attr of node.attributes) {
        ...
        if (pos === undefined && selectionEnd < attr.end) {
            pos = document.positionAt(selectionEnd).character
                -
                    document.positionAt(attr.value.start).character - 1;
        }
        if (pos !== undefined) {
            const [newSelectionStartOffset, newSelectionEndOffset] = findNextWord(attr.value.toString(), pos);
            if (newSelectionStartOffset === undefined ||
                newSelectionEndOffset === undefined) {
                return;
            }
            if (newSelectionStartOffset >= 0 && newSelectionEndOffset >= 0) {
                return offsetRangeToSelection(document, (attr.value.start + newSelectionStartOffset), (attr.value.start + newSelectionEndOffset));
            }
        }
    }
    return;
...
function getPrevAttribute(document: vscode.TextDocument, selectionStart: number, selectionEnd: number, node: HtmlNode): vscode.Selection | undefined {
    ...
    for (let i = node.attributes.length - 1; i >= 0; i--) {
        ...
        const [newSelectionStartOffset, newSelectionEndOffset] = findPrevWord(attr.value.toString(), (selectionStart > attr.value.end
            ? attr.value.toString().length
            : document.positionAt(selectionStart).character
                -
                    document.positionAt(attr.value.start).character));
        if (newSelectionStartOffset === undefined ||
            newSelectionEndOffset === undefined) {
            return;
        }
