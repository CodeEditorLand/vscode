...
import { allowedMimeTypesInScriptTag, getEmbeddedCssNodeIfAny, getEmmetConfiguration, getEmmetHelper, getEmmetMode, getFlatNode, getHtmlFlatNode, getMappingForIncludedLanguages, isOffsetInsideOpenOrCloseTag, isStyleAttribute, isStyleSheet, parsePartialStylesheet, toLSTextDocument, validate, } from "./util";
...
export async function wrapWithAbbreviation(args: any): Promise<boolean> {
    ...
    const operationRanges = Array.from(editor.selections)
        ...
        if (!rangeToReplace.isSingleLine &&
            ...
            rangeToReplace = new vscode.Range(rangeToReplace.start, document.lineAt((rangeToReplace.end.line - 1)).range.end);
        }
        ...
        if (!firstLineOfRange.isEmptyOrWhitespace &&
    ...
    const rangesToReplace: PreviewRangesWithContent[] = operationRanges.map((rangeToReplace) => {
        ...
        const otherMatches = document.lineAt(rangeToReplace.start).text.match(/^(\s*)/);
        textToWrapInPreview = rangeToReplace.isSingleLine
            ? [textToReplace]
            : textToReplace
                .split("\n" +
                (otherMatches ? otherMatches[1] : ""))
                .map((x) => x.trimEnd());
    ...
    const changesWereMade = await (async (inputAbbreviation: string | undefined, previewChanges: boolean): Promise<boolean> => {
        const extractedResults = (!!inputAbbreviation &&
        ...
        if (previewChanges) {
            inPreviewMode = true;
            return ((expandAbbrList: ExpandAbbreviationInput[]): Thenable<boolean> => {
                ...
                return editor.edit((builder) => {
                    ...
                    for (let i = 0; i < rangesToReplace.length; i++) {
                        ...
                        const newLinesInserted = expandedTextLines.length -
                        ...
                        const newPreviewLineEnd = oldPreviewRange.end.line +
        ...
        return expandAbbreviationInRange(editor, rangesToReplace.map((rangesAndContent) => ({
            syntax,
            abbreviation,
            rangeToReplace: rangesAndContent.originalRange,
            textToWrap: rangesAndContent.textToWrapInPreview,
...
export function expandEmmetAbbreviation(args: any): Thenable<boolean | undefined> {
    ...
    if (vscode.window.activeTextEditor.selections.length === 1 &&
        ...
        const prevText = vscode.window.activeTextEditor.document.getText(new vscode.Range(anchor.translate(0, -1), anchor));
    ...
    if (!args["language"]) {
        ...
        if ((vscode.workspace.getConfiguration("emmet")["excludeLanguages"]
    ...
    if (vscode.workspace.getConfiguration("emmet")["triggerExpansionOnTab"] ===
        true &&
    ...
    const getAbbreviation = (document: vscode.TextDocument, selection: vscode.Selection, position: vscode.Position, syntax: string): [
        ...
        if (syntax === "html") {
            const matches = editor.document.lineAt(position.line).text.substr(0, position.character).match(/<(\w+)$/);
            if (matches) {
                abbr = matches[1];
        ...
        return [
            new vscode.Range(abbreviationRange.start.line, abbreviationRange.start.character, abbreviationRange.end.line, abbreviationRange.end.character),
            abbreviation,
            filter,
        ];
    };
    ...
    selectionsInReverseOrder.sort((a, b) => {
        return (a.isReversed ? a.anchor : a.active).compareTo((b.isReversed ? b.anchor : b.active)) * -1;
    });
    ...
    function getRootNode() {
        ...
        if (editor.selections.length === 1 &&
            ...
            rootNode = parsePartialStylesheet(editor.document, editor.selection.isReversed
    ...
    selectionsInReverseOrder.forEach((selection) => {
        ...
        if (validateLocation &&
            !isValidLocationForEmmetAbbreviation(editor.document, getRootNode(), currentNode, syntaxToUse, offset, rangeToReplace)) {
            return;
        }
...
export function isValidLocationForEmmetAbbreviation(document: vscode.TextDocument, rootNode: Node | undefined, currentNode: Node | undefined, syntax: string, offset: number, abbreviationRange: vscode.Range): boolean {
    if (isStyleSheet(syntax)) {
        ...
        if (currentCssNode.parent &&
            ...
            if (document.positionAt(offset).line !== document.positionAt(currentCssNode.selectorToken.end).line &&
...
async function expandAbbreviationInRange(editor: vscode.TextEditor, expandAbbrList: ExpandAbbreviationInput[], insertSameSnippet: boolean): Promise<boolean> {
    ...
    const expandedText = expandAbbr(expandAbbrList[0]);
    if (expandedText) {
        return editor.insertSnippet(new vscode.SnippetString(expandedText), expandAbbrList.map((value) => value.rangeToReplace));
    }
    return false;
}
/**
