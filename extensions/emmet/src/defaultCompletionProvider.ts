...
import { getSyntaxFromArgs, isValidLocationForEmmetAbbreviation, } from "./abbreviationActions";
import { getRootNode } from "./parseDocument";
import { allowedMimeTypesInScriptTag, getEmbeddedCssNodeIfAny, getEmmetConfiguration, getEmmetHelper, getEmmetMode, getFlatNode, getHtmlFlatNode, getMappingForIncludedLanguages, isStyleSheet, parsePartialStylesheet, toLSTextDocument, } from "./util";
export class DefaultCompletionItemProvider implements vscode.CompletionItemProvider {
    ...
    public provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, _: vscode.CancellationToken, context: vscode.CompletionContext): Thenable<vscode.CompletionList | undefined> | undefined {
        ...
        return completionResult.then((completionList) => {
            ...
            else if (expandedText.indexOf(":") > 0 &&
    ...
    private provideCompletionItemsInternal(document: vscode.TextDocument, position: vscode.Position, context: vscode.CompletionContext): Thenable<vscode.CompletionList | undefined> | undefined {
        ...
        const isSyntaxMapped = mappedLanguages[document.languageId]
            ? true
            : false;
        const emmetMode = getEmmetMode(isSyntaxMapped
            ? mappedLanguages[document.languageId]
            : document.languageId, mappedLanguages, excludedLanguages);
        if (!emmetMode ||
        ...
        if (document.getText(new vscode.Range(position.line, 0, position.line, position.character)).trimStart().startsWith("//")) {
            return;
        }
        ...
        if (syntax === "html") {
            ...
            if (validateLocation) {
                ...
                const foundNode = getHtmlFlatNode(document.getText(), getRootNode(document, true), positionOffset, false);
                if (foundNode) {
                    if (foundNode.name === "script") {
                        ...
                        if (typeNode) {
                            ...
                            if (typeAttrValue === "application/javascript" ||
                                typeAttrValue === "text/javascript") {
                                if (!getSyntaxFromArgs({
                                    language: "javascript",
                                })) {
                                    return;
                                }
        ...
        const extractAbbreviationResults = helper.extractAbbreviation(toLSTextDocument(document), position, (isStyleSheet(syntax)
        ...
        if (!isStyleSheet(document.languageId) &&
            ...
            currentNode = getFlatNode(getEmbeddedCssNodeIfAny(document, getFlatNode(rootNode, offset, true), position), offset, true);
        }
        if (validateLocation &&
            !isValidLocationForEmmetAbbreviation(document, rootNode, currentNode, syntax, offset, toRange(extractAbbreviationResults.abbreviationRange))) {
            return;
        }
        ...
        if (!isStyleSheet(syntax) &&
            ...
            if (abbreviation.startsWith("this.") ||
                ...
                isNoisePromise = vscode.commands
                    .executeCommand<vscode.SymbolInformation[] | undefined>("vscode.executeDocumentSymbolProvider", document.uri)
                    .then((symbols) => {
                    return (!!symbols &&
                        symbols.some((x) => abbreviation === x.name ||
        ...
        return isNoisePromise.then((isNoise): vscode.CompletionList | undefined => {
            ...
            const result = helper.doComplete(toLSTextDocument(document), position, syntax, getEmmetConfiguration(syntax!));
