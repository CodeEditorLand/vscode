...
import { findNextWord, findPrevWord, getDeepestFlatNode, getFlatNode, offsetRangeToSelection, } from "./util";
...
function getSelectionFromProperty(document: vscode.TextDocument, node: Node | undefined, selectionStart: number, selectionEnd: number, selectFullValue: boolean, direction: string): vscode.Selection | undefined {
        ...
        pos =
            selectionStart > propertyNode.valueToken.end
                ...
                : document.positionAt(selectionStart).character
                    -
                        document.positionAt(propertyNode.valueToken.start).character;
    }
        ...
        pos =
            selectionEnd === propertyNode.valueToken.end
                ...
                : document.positionAt(selectionEnd).character
                    -
                        document.positionAt(propertyNode.valueToken.start).character - 1;
    }
    const [newSelectionStartOffset, newSelectionEndOffset] = direction === "prev"
    ...
    return new vscode.Selection(tokenStart.translate(0, newSelectionStartOffset), tokenStart.translate(0, newSelectionEndOffset));
}
