...
export function evaluateMathExpression(): Thenable<boolean> {
    ...
    return editor.edit((editBuilder) => {
        editor.selections.forEach((selection) => {
            ...
            try {
                if (selectionText) {
                    editBuilder.replace(new vscode.Range(startpos, endpos), String(evaluate(selectionText)));
                }
                else {
                    ...
                    editBuilder.replace(new vscode.Range(new vscode.Position(selection.end.line, extractedIndices[0]), new vscode.Position(selection.end.line, extractedIndices[1])), String(evaluate(lineToSelectionEnd.substr(extractedIndices[0], extractedIndices[1]))));
                }
            }
            catch (err) {
