...
import { getEmmetMode, getMappingForIncludedLanguages, getPathBaseName, getSyntaxes, LANGUAGE_MODES, migrateEmmetExtensionsPath, updateEmmetExtensionsPath, } from "./util";
export function activateEmmetExtension(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((e) => {
        ...
        if (basefileName.startsWith("snippets") &&
    ...
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument((e) => {
        ...
        if (syntaxes.markup.includes(emmetMode) ||
...
function refreshCompletionProviders(_: vscode.ExtensionContext) {
    ...
    const inlineCompletionProvider: vscode.InlineCompletionItemProvider = {
        async provideInlineCompletionItems(document: vscode.TextDocument, position: vscode.Position, _: vscode.InlineCompletionContext, token: vscode.CancellationToken) {
            ...
            if (document.getText(item.range as vscode.Range) !== item.label) {
    ...
    Object.keys(includedLanguages).forEach((language) => {
        ...
        if (useInlineCompletionProvider) {
            completionProviderDisposables.push(vscode.languages.registerInlineCompletionItemProvider({ language, scheme: "*" }, inlineCompletionProvider));
        }
        completionProviderDisposables.push(vscode.languages.registerCompletionItemProvider({ language, scheme: "*" }, completionProvider, ...LANGUAGE_MODES[includedLanguages[language]]));
    ...
    Object.keys(LANGUAGE_MODES).forEach((language) => {
        if (!languageMappingForCompletionProviders.has(language)) {
            ...
            completionProviderDisposables.push(vscode.languages.registerCompletionItemProvider({ language, scheme: "*" }, completionProvider, ...LANGUAGE_MODES[language]));
