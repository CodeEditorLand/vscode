...
class BranchDeleteItem extends RefItem {
	async run(repository: Repository, force?: boolean): Promise<void> {
		if (this.ref.type === RefType.Head && this.refName) {
			...
			await repository.deleteRemoteRef(this.refRemote, refName, {
				force,
			});
		}
	}
...
export class CommandCenter {
	...
	async stageSelectedChanges(changes: LineChange[]): Promise<void> {
		...
		const workingTreeLineChanges = toLineChanges(
			workingTreeDiffInformation,
		);

		this.logger.trace(
			`[CommandCenter][stageSelectedChanges] changes: ${JSON.stringify(changes)}`,
		);
		this.logger.trace(
	...
	async revertSelectedRanges(changes: LineChange[]): Promise<void> {
		...
		this.logger.trace(
			`[CommandCenter][revertSelectedRanges] changes: ${JSON.stringify(changes)}`,
		);
		this.logger.trace(
	...
	async unstageSelectedRanges(changes: LineChange[]): Promise<void> {
		...
		this.logger.trace(
			`[CommandCenter][unstageSelectedRanges] changes: ${JSON.stringify(changes)}`,
		);
		this.logger.trace(
	...
	async checkout2(
		...
		if (
			...
			await repository.checkout(historyItemRef.name, {
				pullBeforeCheckout,
			});
			return;
		}
		...
		// Remote branch
		const branches = await repository.findTrackingBranches(
			historyItemRef.name,
		);
		if (branches.length > 0) {
	...
	async deleteBranch2(
		...
		await this._deleteBranch(repository, remoteName, refName, {
			remote: true,
		});
	}
	...
	private async _deleteBranch(
		...
		if (!options.remote && typeof name === "string") {
			...
			// Remote branch
			run = (force) =>
				repository.deleteRemoteRef(remote, name!, { force });
		} else {
			const getBranchPicks = async () => {
				...
				return refs
					.filter(
						(ref) => ref.name && !refsToExclude.includes(ref.name),
					)
	...
	async viewCommit(
		...
		const changes = await repository.diffTrees(
			historyItemParentId,
			historyItemId,
		);
		const resources = changes.map((c) =>
