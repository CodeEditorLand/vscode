...
export class Repository implements Disposable {
	...
	provideOriginalResource(uri: Uri): Uri | undefined {
		...
		if (
			this.mergeGroup.resourceStates.some((r) =>
		...
		if (
			this.untrackedGroup.resourceStates.some((r) =>
		...
		if (
			...
			activeTabInput.textDiffs.some((diff) =>
				pathEquals(diff.modified.fsPath, uri.fsPath),
			)
		) {
			this.logger.trace(
	...
	private async getStatus(
		...
		if (didHitLimit && !shouldIgnore && !this.didWarnAboutLimit) {
			...
			if (knownHugeFolderPaths.length > 0) {
				...
				window
					.showWarningMessage(gitWarn, ok, neverAgain)
					.then((result) => {
						if (result === neverAgain) {
