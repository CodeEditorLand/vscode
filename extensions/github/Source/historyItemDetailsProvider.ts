	...
	implements SourceControlHistoryItemDetailsProvider
{
		...
		try {
			...
			await Promise.all(
				authorQuery.map(async (commits) => {
					...
					if (
						commits.some(({ hash }) =>
							repositoryStore.commits.has(hash),
						)
					) {
						commits.forEach(({ hash }) =>
					...
					commits.forEach(({ hash }) =>
						results.set(
							hash,
							`${commitAuthor.avatarUrl}&s=${query.size}`,
						),
					);
				}),
			);
	...
	private async _loadAssignableUsers(descriptor: {
		...
		try {
			...
			}>(ASSIGNABLE_USERS_QUERY, descriptor);
	...
	private async _getCommitAuthor(
		...
		try {
			...
			const { repository } = await graphql<{
				repository: GitHubRepository;
			}>(COMMIT_AUTHOR_QUERY, { ...descriptor, commit });
