...
function removeAnsiEscapeCodes(str: string): string {
    if (str) {
        str = str.replace(/(?:(?:\x1b\[|\x9B)[=?>!]?[\d;:]*["$#'* ]?[a-zA-Z@^`{}|~])|(:?\x1b\].*?\x07)/g, "");
    }
    return str;
}
class Trigger {
...
class ServerReadyDetector extends vscode.Disposable {
        ...
        if (!this.terminalDataListener) {
            this.terminalDataListener = vscode.window.onDidWriteTerminalData(async (e) => {
                ...
                for (const [, detector] of this.detectors) {
                    if (detector.shellPid ===
                        await e.terminal.processId) {
    ...
    private constructor(private session: vscode.DebugSession) {
        ...
        this.regexp = new RegExp(session.configuration.serverReadyAction.pattern ||
            "listening on.* (https?://\\S+|[0-9]+)", "i");
    }
    ...
    private openExternalWithString(session: vscode.DebugSession, captureString: string) {
        ...
        if (captureString === "") {
            ...
            if (format.indexOf("%s") >= 0) {
                vscode.window
                    .showErrorMessage(vscode.l10n.t("Format uri ('{0}') uses a substitution placeholder but pattern did not capture anything.", format), { modal: true })
                    .then((_) => undefined);
                return;
            }
            uri = format;
            ...
            const format = args.uriFormat ||
                (/^[0-9]+$/.test(captureString) ?
                    "http://localhost:%s"
                    :
                        "%s");
            if (format.split("%s").length !== 2) {
                vscode.window
                    .showErrorMessage(vscode.l10n.t("Format uri ('{0}') must contain exactly one substitution placeholder.", format), { modal: true })
                    .then((_) => undefined);
                return;
            }
    ...
    private async debugWithBrowser(type: string, session: vscode.DebugSession, uri: string) {
        if (!(session.configuration
            .serverReadyAction as ServerReadyAction).killOnServerStop) {
        ...
        const createdSession = await this.catchStartedDebugSession((session) => session.configuration._debugServerReadySessionId === trackerId, cts.token);
        cts.dispose();
        if (!createdSession) {
            return;
        }
    ...
    private startBrowserDebugSession(type: string, session: vscode.DebugSession, uri: string, trackerId?: string) {
        return vscode.debug.startDebugging(session.workspaceFolder, {
            ...
            request: "launch",
            url: uri,
            webRoot: session.configuration.serverReadyAction.webRoot ||
                "${workspaceFolder}",
    ...
    private async startDebugSession(session: vscode.DebugSession, name: string, config?: vscode.DebugConfiguration) {
        ...
        const createdSession = await this.catchStartedDebugSession((x) => x.name === name, cts.token);
        cts.dispose();
        if (!createdSession) {
            return;
        }
    ...
    private catchStartedDebugSession(predicate: (session: vscode.DebugSession) => boolean, cancellationToken: vscode.CancellationToken): Promise<vscode.DebugSession | undefined> {
        return new Promise<vscode.DebugSession | undefined>((_resolve) => {
            const done = (value?: vscode.DebugSession) => {
                ...
                cancellationToken.onCancellationRequested(done).dispose();
                this.disposables.delete(vscode.debug.onDidStartDebugSession((session) => {
                    if (predicate(session)) {
                        done(session);
                    }
                }));
                this.disposables.delete(cancellationToken.onCancellationRequested(done));
                _resolve(value);
            };
            // In case the debug session of interest was never caught anyhow.
            this.disposables.add(vscode.debug.onDidStartDebugSession((session) => {
                if (predicate(session)) {
                    done(session);
                }
            }));
            this.disposables.add(cancellationToken.onCancellationRequested(done));
        });
    }
}
export function activate(context: vscode.ExtensionContext) {
    context.subscriptions.push(vscode.debug.onDidStartDebugSession((session) => {
        if (session.configuration.serverReadyAction) {
            if (ServerReadyDetector.start(session)) {
