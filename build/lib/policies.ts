...
class BooleanPolicy extends BasePolicy {
    static from(name: string, category: Category, minimumVersion: string, description: NlsString, moduleName: string, settingNode: Parser.SyntaxNode): BooleanPolicy | undefined {
        if (getStringProperty(settingNode, "type")
            !== "boolean") {
            return undefined;
        }
        return new BooleanPolicy(name, category, minimumVersion, description, moduleName);
...
class ObjectPolicy extends BasePolicy {
    static from(name: string, category: Category, minimumVersion: string, description: NlsString, moduleName: string, settingNode: Parser.SyntaxNode): ObjectPolicy | undefined {
        const type = getStringProperty(settingNode, "type");
        if (type !== "object" && type !== "array") {
            return undefined;
        }
...
class StringEnumPolicy extends BasePolicy {
    static from(name: string, category: Category, minimumVersion: string, description: NlsString, moduleName: string, settingNode: Parser.SyntaxNode): StringEnumPolicy | undefined {
        ...
        if (!((value: (string | NlsString)[]): value is string[] => {
            return !value.some((s) => isNlsString(s));
        })(enum_)) {
            throw new Error(`Property 'enum' should not be localized.`);
        }
        ...
        else if (!((value: (string | NlsString)[]): value is NlsString[] => {
            return value.every((s) => isNlsString(s));
        })(enumDescriptions)) {
            throw new Error(`Property 'enumDescriptions' should be localized.`);
        }
...
const StringQ: QType<string | NlsString> = {
    ...
    value(matches: Parser.QueryMatch[]): string | NlsString | undefined {
        ...
        if (match.captures.filter((c) => c.name === "nlsKey")[0]
            ?.node.text) {
            return { value, nlsKey };
        }
        else {
...
function getProperty<T>(qtype: QType<T>, node: Parser.SyntaxNode, key: string): T | undefined {
    return qtype.value(new Parser.Query(typescript, `(
			(pair
...
if (require.main === module) {
    (async () => {
        const [policies, translations] = await Promise.all([
            (async (): Promise<Policy[]> => {
                ...
                const policies = [];
                for (const file of await (async (root: string): Promise<string[]> => {
                    return new Promise((c, e) => {
                        const result: string[] = [];
                        const stream = byline(spawn(rgPath, [
                            "-l",
                    ...
                    policies.push(...((moduleName: string, node: Parser.SyntaxNode): Policy[] => {
                        return new Parser.Query(typescript, `
		(
			(call_expression
	...
	`).matches(node).map((m) => {
                            return ((moduleName: string, configurationNode: Parser.SyntaxNode, settingNode: Parser.SyntaxNode, policyNode: Parser.SyntaxNode, categories: Map<string, Category>): Policy => {
                                ...
                                return result;
                            })(moduleName, m.captures.filter((c) => c.name === "configuration")[0].node, m.captures.filter((c) => c.name === "setting")[0]
                                .node, m.captures.filter((c) => c.name === "policy")[0]
                                .node, new Map<string, Category>());
                        });
                    })(path
                        .relative(path.join(process.cwd(), "src"), file)
                        .replace(/\.ts$/i, "")
                        .replace(/\\/g, "/"), parser.parse(await fs.readFile(file, { encoding: "utf8" })).rootNode));
                }
                return policies;
            })(),
            (async (): Promise<Translations> => {
                ...
                return await Promise.all(Object.keys(Languages).map((languageId) => (async (extensionGalleryServiceUrl: string, resourceUrlTemplate: string, languageId: string, version: Version) => {
                    const latestCompatibleVersion = (await (async (serviceUrl: string, languageId: string): Promise<Version[]> => {
                    ...
                    })(extensionGalleryServiceUrl, languageId)).filter((v) => compareVersions(v, [version[0], version[1] + 1, 0]) < 0).at(-1)!; // order is newest to oldest
                    if (!latestCompatibleVersion) {
                        throw new Error(`No compatible language pack found for ${languageId} for version ${version}`);
                    }
                    return await (async (resourceUrlTemplate: string, languageId: string, version: Version) => {
                        const resource = {
                            publisher: "ms-ceintl",
                        ...
                        const res = await fetch(resourceUrlTemplate.replace(/\{([^}]+)\}/g, (_, key) => resource[key as keyof typeof resource]));
                        if (res.status !== 200) {
                        ...
                        return result;
                    })(resourceUrlTemplate, languageId, latestCompatibleVersion);
                })(extensionGalleryServiceUrl, resourceUrlTemplate, languageId, parseVersion(require("../../package.json").version)).then((languageTranslations) => ({
                    languageId,
                    languageTranslations,
                }))));
            })(),
        ]);
        const { admx, adml } = await ((policies: Policy[], translations: Translations) => {
            ...
            return {
                admx: ((regKey: string, versions: string[], categories: Category[], policies: Policy[]) => {
...
`;
                })(product.win32RegValueName, versions, categories, policies),
                adml: [
                    {
                        languageId: "en-us",
