...
export function create(projectPath: string, existingOptions: Partial<ts.CompilerOptions>, config: {
...
}, onError: (message: string) => void = (err: string) => console.log(JSON.stringify(err, null, 4))): IncrementalCompiler {
    ...
    if (config.transpileOnly) {
        result = <any>(() => createTranspileStream((!config.transpileWithSwc
        ...
        result = <any>(((token: builder.CancellationToken) => createCompileStream(builder.createTypeScriptBuilder({ logFn }, projectPath, cmdLine), token)));
    }
    result.src = (opts?: {
        cwd?: string;
        base?: string;
