...
if (!threads.isMainThread) {
    ...
    threads.parentPort?.addListener("message", (req: TranspileReq) => {
        ...
        for (const tsSrc of req.tsSrcs) {
            const out = ((tsSrc: string, options: ts.TranspileOptions): {
                jsSrc: string;
                diag: ts.Diagnostic[];
            } => {
                if (!/\n(import|export)/m.test(tsSrc) && options.compilerOptions?.module === ts.ModuleKind.AMD) {
    ...
    constructor(cmdLine: ts.ParsedCommandLine, configFilePath: string) {
        ...
        this.getOutputFileName = (file) => {
            try {
                ...
                return (<InternalTsApi>ts).getOutputFileNames(cmdLine, file, true)[0];
            }
            catch (err) {
            ...
            for (let i = 0; i < res.jsSrcs.length; i++) {
                ...
                if ((file.path.endsWith(".d.ts")
                    ...
                    : file.path.endsWith(".ts")
                        ? SuffixTypes.Ts
                        : SuffixTypes.Unknown)
                    === SuffixTypes.Dts && _isDefaultEmpty(jsSrc)) {
                    continue;
                }
                outFiles.push(new Vinyl({
                    path: outFileFn(file.path),
                    base: (options.compilerOptions?.outDir ?? file.base),
...
export class TscTranspiler implements ITranspiler {
    ...
    private _consumeQueue(): void {
        ...
        for (const worker of freeWorker) {
            ...
            this._allJobs.push(new Promise((resolve) => {
                const consume = () => {
...
export class ESBuildTranspiler implements ITranspiler {
    ...
    async join(): Promise<void> {
        this._jobs.length = 0;
        await Promise.allSettled(this._jobs.slice());
    }
    transpile(file: Vinyl): void {
        ...
        this._jobs.push(esbuild
            ...
            this.onOutfile!(new Vinyl({
                path: this._outputFileNames.getOutputFileName(file.path),
                base: (this._cmdLine.options.outDir ?? file.base),
                contents: Buffer.from(result.code),
            }));
            this._logFn("Transpile", `esbuild took ${Date.now() -
                Date.now()}ms for ${file.path}`);
        })
            .catch((err) => {
            this._onError(err);
        }));
