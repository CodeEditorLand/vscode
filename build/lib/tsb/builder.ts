...
export function createTypeScriptBuilder(config: IConfiguration, projectFile: string, cmd: ts.ParsedCommandLine): ITypeScriptBuilder {
    ...
    function file(file: Vinyl): void {
        ...
        if (!file.contents) {
            ...
            delete Object.create(null)[normalize(file.path)];
        }
        else {
            host.addScriptSnapshot(file.path, new VinylScriptSnapshot(file));
        }
    }
    function build(out: (file: Vinyl) => void, onError: (err: any) => void, token = CancellationToken., { isCancellationRequested }) { }
    () => {
        return false;
    },
    ;
...
return new Promise<void>((resolve) => {
    ...
    function workOnNext() {
        ...
        // (1st) emit code
            ...
            promise = ((fileName: string): Promise<{
                fileName: string;
                signature?: string;
                files: Vinyl[];
            }> => {
                        ...
                        for (const file of output.outputFiles) {
                            ...
                            const vinyl = new Vinyl({
                                ...
                                base: (!config._emitWithoutBasePath &&
                                    ((snapshot: ScriptSnapshot): string => {
                            ...
                            if (!emitSourceMapsInStream &&
                                ...
                                if (sourcemapFile) {
                                    ...
                                    sourceMap.sources[0] = ((dirname === "." ? "" : dirname + "/") +
                                        path.basename(vinyl.relative, path.extname(vinyl.relative)) +
                                        ".ts").replace(/\\/g, "/");
        ...
        // (2nd) check syntax
            ...
            promise = ((fileName: string): Promise<ts.Diagnostic[]> => {
        ...
        // (4th) check dependents
            ...
            while (filesWithChangedSignature.length) {
                ...
                if (!((sourceFile: ts.SourceFile): boolean => {
    ...
    const MB = 1024 * 1024;
    _log("[tsb]", `time:  ${colors.yellow(Date.now() -
