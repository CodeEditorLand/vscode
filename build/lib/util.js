...
function incremental(streamProvider, initial, supportsCancellation) {
	...
	const run = (input, isCancellable) => {
		...
		input
			.pipe(stream)
			.pipe(
				event_stream_1.default.through(undefined, () => {
					state = "idle";
					eventuallyRun();
				}),
			)
...
function cleanNodeModules(rulePath) {
	...
	const input = event_stream_1.default.through();
	const output = event_stream_1.default.merge(
		input.pipe((0, gulp_filter_1.default)(["**", ...excludes])),
		input.pipe((0, gulp_filter_1.default)(includes)),
	);
	return event_stream_1.default.duplex(input, output);
}
...
function _rreaddir(dirPath, prepend, result) {
	...
	for (const entry of entries) {
		if (entry.isDirectory()) {
			_rreaddir(
				path_1.default.join(dirPath, entry.name),
				`${prepend}/${entry.name}`,
				result,
			);
		} else {
