...
async function _execute(task: Task): Promise<void> {
    ...
    const elapsedArr = process.hrtime(process.hrtime());
    if (!task._tasks) {
        fancyLog(`Finished`, ansiColors.cyan(name), "after", ansiColors.magenta(((time: number): string => {
            return `${Math.round(time)} ms`;
        })((elapsedArr[0] * 1e9 + elapsedArr[1])
            / 1e6)));
    }
}
async function _doExecute(task: Task): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (((p: Promise<void> | NodeJS.ReadWriteStream): p is Promise<void> => {
