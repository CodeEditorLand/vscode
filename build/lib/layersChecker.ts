...
//
const program = ((tsconfigPath: string): ts.Program => {
    const tsConfigParsed = ts.parseJsonConfigFileContent(ts.readConfigFile(tsconfigPath, ts.sys.readFile).config, {
        fileExists: existsSync,
        readDirectory: ts.sys.readDirectory,
        readFile: (file) => readFileSync(file, "utf8"),
        useCaseSensitiveFileNames: process.platform === "linux",
    }, resolve(dirname(tsconfigPath)), { noEmit: true });
    return ts.createProgram(tsConfigParsed.fileNames, tsConfigParsed.options, ts.createCompilerHost(tsConfigParsed.options, true));
})(join(__dirname, "../../", "src", "tsconfig.json"));
for (const sourceFile of program.getSourceFiles()) {
    for (const rule of RULES) {
        if (match([sourceFile.fileName], rule.target).length > 0) {
            if (!rule.skip) {
                ((program: ts.Program, sourceFile: ts.SourceFile, rule: IRule) => {
                    ((node: ts.Node): void => {
                        ...
                        const text = (_parentSymbol as ts.Symbol).getName();
