...
module.exports = function (pattern: string | string[] | filter.FileFunction, options?: {
    ...
    if (!watcher) {
        watcher = cache[cwd] = ((root: string): Stream => {
            const result = es.through();
            let child: cp.ChildProcess | null = cp.spawn(path.join(__dirname, "watcher.exe"), [root]);
            child.stdout!.on("data", function (data) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    const file = new File({
                        path: path.join(root, changePath),
                        base: root,
                    });
                    (<any>file).event = ((type: "0" | "1" | "2"): "change" | "add" | "unlink" => {
                        switch (type) {
                            case "0":
                                return "change";
                            case "1":
