        ...
        for (const [name, info] of data.fields) {
            ...
            while (parent) {
                if (parent.fields.get(name)?.type === FieldType.Protected) {
                    reportViolation(name, `'${name}' from ${parent.fileName}:${parent.node
                        .getSourceFile()
                        .getLineAndCharacterOfPosition(parent.fields.get(name)!.pos).line + 1}`, `${data.fileName}:${data.node
                        .getSourceFile()
                        .getLineAndCharacterOfPosition(info.pos).line + 1}`);
    ...
    static fillInReplacement(data: ClassData) {
        ...
        for (const [name, info] of data.fields) {
            if (ClassData._shouldMangle(info.type)) {
                data.replacements.set(name, new ShortIdent("").next((name: string) => {
                    // locally taken
...
if (__filename === argv[1]) {
    (async () => {
        const projectBase = path.join(path.join(__dirname, "..", "..", ".."), "src");
        ...
        for (const [fileName, contents] of await new Mangler(path.join(projectBase, "tsconfig.json"), console.log, {
            mangleExports: true,
