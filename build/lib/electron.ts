...
const darwinCreditsTemplate = product.darwinCredits &&
    ((input: string): (params: Record<string, string>) => string => {
...
function darwinBundleDocumentType(extensions: string[], icon: string, nameOrSuffix?: string | DarwinDocumentSuffix, utis?: string[]): DarwinDocumentType {
    ...
    if (((str?: string): str is DarwinDocumentSuffix => {
        return (str === "document" ||
            str === "script" ||
            str === "file" ||
            str === "source code");
    })(nameOrSuffix) || !nameOrSuffix) {
        nameOrSuffix =
...
if (require.main === module) {
    (async (arch: string = process.arch): Promise<void> => {
        const electronPath = path.join(root, ".build", "electron");
        const versionFile = path.join(electronPath, "version");
        if (!(fs.existsSync(versionFile) &&
            fs.readFileSync(versionFile, "utf8") === `${electronVersion}`)) {
            await util.rimraf(electronPath)();
            await util.streamToPromise(((arch: string): () => NodeJS.ReadWriteStream => {
                return () => {
                    return vfs
                        .src("package.json")
                        .pipe((require("gulp-json-editor") as typeof import("gulp-json-editor"))({ name: product.nameShort }))
                        .pipe(require("@vscode/gulp-electron")({
                        ...config,
