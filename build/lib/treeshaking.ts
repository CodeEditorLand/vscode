...
function createTypeScriptLanguageService(ts: typeof import("typescript"), options: ITreeShakingOptions): ts.LanguageService {
    ...
    options.typings.forEach((typing) => {
        FILES[typing] = fs.readFileSync(path.join(options.sourcesRoot, typing)).toString();
    });
    return ts.createLanguageService(new TypeScriptLanguageServiceHost(ts, processLibFiles(ts, options), FILES, ts.convertCompilerOptionsFromJson(options.compilerOptions, options.sourcesRoot).options));
}
/**
...
function discoverAndReadFiles(ts: typeof import("typescript"), options: ITreeShakingOptions): IFileMap {
    ...
    while (queue.length > 0) {
        ...
        if (fs.existsSync(dts_filename)) {
            FILES[`${moduleId}.d.ts`] =
                fs.readFileSync(dts_filename).toString();
            continue;
        }
        if (fs.existsSync(path.join(options.sourcesRoot, redirectedModuleId + ".js"))) {
            // This is an import for a .js file, so ignore it...
            continue;
        }
        const ts_filecontents = fs.readFileSync(path.join(options.sourcesRoot, redirectedModuleId + ".ts")).toString();
...
function processLibFiles(ts: typeof import("typescript"), options: ITreeShakingOptions): ILibMap {
    ...
    while (stack.length > 0) {
        ...
        if (!result[key]) {
            const sourceText = fs.readFileSync(path.join(path.dirname(require.resolve("typescript/lib/lib.d.ts")), filename)).toString();
            result[key] = sourceText;
            for (const ref of ts.preProcessFile(sourceText).libReferenceDirectives) {
...
function markNodes(ts: typeof import("typescript"), languageService: ts.LanguageService, options: ITreeShakingOptions) {
    ...
    function enqueue_black(node: ts.Node): void {
                ...
                sourceFile.forEachChild((node: ts.Node) => {
                    if (ts.isImportDeclaration(node)) {
                        if (!node.importClause &&
                            ...
                            setColor(node, NodeColor.Black);
                            ((node: ts.Node, importText: string): void => {
                                ...
                                if (/(^\.\/)|(^\.\.\/)/.test(importText)) {
                                    ...
                                    fullPath =
                                        path.join(path.dirname(node.getSourceFile().fileName), importText) +
                                            ".ts";
                                }
                                else {
                            ...
                            })(node, node.moduleSpecifier.text);
                        }
                        return;
                    }
                    ...
                    if (((ts: typeof import("typescript"), node: ts.Node): boolean => {
                        ...
                        const visitNode = (node: ts.Node) => {
                            ...
                            if (ts.isCallExpression(node) || ts.isNewExpression(node)) {
                                if (!/(createDecorator|refineServiceDecorator)/.test(node.expression.getText())) {
                                    hasSideEffects = true;
                                }
                            }
        ...
        setColor(node, NodeColor.Black);
        black_queue.push(node);
        if (options.shakeLevel === ShakeLevel.ClassMembers &&
            ...
            if (references) {
                for (let i = 0, len = references.length; i < references.length; i++) {
    ...
    while (black_queue.length > 0 || gray_queue.length > 0) {
        ...
        const loop = (node: ts.Node) => {
            for (const { symbol, symbolImportNode } of getRealNodeSymbol(ts, checker, node)) {
                if (symbolImportNode) {
                    ...
                    if (importDeclarationNode &&
                        ...
                        ((node: ts.Node, importText: string): void => {
                            ...
                            enqueueFile(fullPath);
                        })(importDeclarationNode, importDeclarationNode.moduleSpecifier.text);
                    }
                }
                if (((symbol: ts.Symbol | undefined | null): symbol is ts.Symbol & {
                    declarations: ts.Declaration[];
                } => {
                    return !!(symbol && symbol.declarations);
                })(symbol) &&
                    !nodeIsInItsOwnDeclaration(node.getSourceFile(), node, symbol)) {
                    for (let i = 0, len = symbol.declarations.length; i <
                        symbol.declarations.length; i++) {
                        ...
                        if (options.shakeLevel === ShakeLevel.ClassMembers &&
                            ...
                            for (let j = 0; j < declaration.members.length; j++) {
                                ...
                                if (((ts: typeof import("typescript"), node: ts.ClassElement | ts.TypeElement): boolean => {
...
function nodeIsInItsOwnDeclaration(nodeSourceFile: ts.SourceFile, node: ts.Node, symbol: ts.Symbol & {
    ...
    for (let i = 0, len = symbol.declarations.length; i < symbol.declarations.length; i++) {
        const declaration = symbol.declarations[i];
        if (nodeSourceFile ===
            ...
            if (ts.isImportDeclaration(node)) {
                if (node.importClause && node.importClause.namedBindings) {
                    if (ts.isNamespaceImport(node.importClause.namedBindings)) {
                        ...
                        const leadingTrivia = sourceFile.text.substr(node.pos, node.getLeadingTriviaWidth());
            ...
            if (ts.isExportDeclaration(node)) {
                if (node.exportClause &&
                    ...
                    if (survivingExports.length > 0) {
                        return write(`${sourceFile.text.substr(node.pos, node.getLeadingTriviaWidth())}export {${survivingExports.join(",")} } from${node.moduleSpecifier.getFullText(sourceFile)};`);
                    }
                }
            }
            if (shakeLevel === ShakeLevel.ClassMembers &&
                ...
                for (let i = node.members.length - 1; i >= 0; i--) {
                    ...
                    toWrite =
                        toWrite.substring(0, (member.pos - node.pos)) + toWrite.substring((member.end - node.pos));
                }
...
function getRealNodeSymbol(ts: typeof import("typescript"), checker: ts.TypeChecker, node: ts.Node): SymbolImportTuple[] {
    ...
    if (symbol) {
        ...
        if (ts.isPropertyName(node) &&
            ...
            const name = (<any>ts).getNameFromPropertyName(node);
        ...
        const element = (<any>ts)
            .getContainingObjectLiteralElement(node);
        if (element) {
            const contextualType = element && checker.getContextualType(element.parent);
            if (contextualType) {
                const propertySymbols = (<any>ts)
                    .getPropertySymbolsFromContextualType(element, checker, contextualType, 
                /*unionSymbolOk*/ false);
                if (propertySymbols) {
