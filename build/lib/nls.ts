...
export function nls(options: {
    ...
    let base: string;
    const output = through().pipe(sort()) // IMPORTANT: to ensure stable NLS metadata generation, we must sort the files because NLS messages are globally extracted and indexed across all files
        ...
        contents: Buffer.from(JSON.stringify(_nls.[])),
        base,
 ...
 * Copyright (C) Microsoft Corporation. All rights reserved.
 *--------------------------------------------------------*/
globalThis._VSCODE_NLS_MESSAGES=${JSON.stringify(_nls.[])};`),
        base,
...
module _nls {
    ...
    function analyze(ts: typeof import("typescript"), contents: string, functionName: "localize" | "localize2", options: ts.CompilerOptions = {}): ILocalizeAnalysisResult {
        ...
        const service = ts.createLanguageService(new SingleFileServiceHost(ts, Object.assign(clone(options), { noResolve: true }), filename, contents));
        const sourceFile = ts.createSourceFile(filename, contents, ts.ScriptTarget.ES5, true);
        // all imports
        const imports = lazy(collect(ts, sourceFile, (n) => ((ts: typeof import("typescript"), node: ts.Node): boolean => {
        ...
        const nlsLocalizeCallExpressions = importDeclarations
            ...
            .filter((d) => (<ts.ExternalModuleReference>d.moduleReference).expression
            .getText()
            .endsWith(`/nls.js'`)).map((d) => d.name))
            ...
            (<ts.PropertyAccessExpression>(n.expression)).name.getText() === functionName);
        ...
        const localizeCalls = nlsLocalizeCallExpressions
            .concat(allLocalizeImportDeclarations
            .filter((d) => d.name.getText() === functionName)
            .map((n) => service.getReferencesAtPosition(filename, n.pos + 1))
            .flatten()
            .filter((r) => !r.isWriteAccess).concat(allLocalizeImportDeclarations
    ...
    class TextModel {
        ...
        public apply(patch: IPatch): void {
            ...
            this.lines[startLineNumber] = [
                (this.lines[startLineNumber] || "").substring(0, patch.span.start.character),
                patch.content,
                (this.lines[endLineNumber] || "").substring(patch.span.end.character),
            ].join("");
    ...
    export function patchFile(javascriptFile: File, typescript: string, options: {
        ...
        const moduleId = javascriptFile.relative
            .replace(/\.js$/, "")
            .replace(/\\/g, "/");
        const { javascript, sourcemap, nlsKeys, nlsMessages } = ((ts: typeof import("typescript"), typescript: string, javascript: string, sourcemap: sm.RawSourceMap, options: {
            preserveEnglish: boolean;
        }): INlsPatchResult => {
            ...
            const patches = lazy(localizeCalls)
                ...
                .flatten()
                .map(toPatch).concat(lazy(localize2Calls)
                .map((lc) => ({
                range: lc.keySpan,
                content: `${allNLSMessagesIndex++}`,
            ...
            javascript = ((patches: IPatch[], contents: string): string => {
            ...
            sourcemap = ((patches: IPatch[], rsm: sm.RawSourceMap, smc: sm.SourceMapConsumer): sm.RawSourceMap => {
                ...
                smc.eachMapping((m) => {
                    ...
                    if (patch &&
                        ...
                        const lengthDiff = patch.content.length
                            -
            ...
            })(patches, sourcemap, smc);
            return { javascript, sourcemap, nlsKeys, nlsMessages };
        })(require("typescript") as typeof import("typescript"), typescript, javascriptFile.contents.toString(), (<any>javascriptFile).sourceMap, options);
        const result = ((file: File, contents: string, path: string = file.path) => {
            return new File({
