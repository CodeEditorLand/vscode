	...
	for (const result of results) {
		if (result.body) {
			...
			fs_1.default.writeFileSync(
				path_1.default.join(extensionFolder, result.fileName),
				result.body,
			);
			console.log(
				`  - ${result.fileName} ${ansi_colors_1.default.green("✔︎")}`,
			);
		} else if (result.body === undefined) {
			console.log(
				`  - ${result.fileName} ${ansi_colors_1.default.yellow("⚠️")}`,
			);
		} else {
			console.log(
				`  - ${result.fileName} ${ansi_colors_1.default.red("🛑")}`,
			);
		}
	}
	// Validation
...
main().then(
	() => {
		console.log(
			`Built-in extensions component data downloaded ${ansi_colors_1.default.green("✔︎")}`,
		);
		process.exit(0);
	},
	(err) => {
		console.log(
			`Built-in extensions component data could not be downloaded ${ansi_colors_1.default.red("🛑")}`,
		);
		console.error(err);
		process.exit(1);
	},
