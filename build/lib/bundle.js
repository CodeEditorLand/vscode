...
function bundle(entryPoints, config, callback) {
	...
	const code = require("fs").readFileSync(
		path_1.default.join(__dirname, "../../src/vs/loader.js"),
	);
	const r = vm_1.default.runInThisContext(
		...
		deps = deps.map((dep) => {
			...
			if (/^\.\//.test(_path) || /^\.\.\//.test(_path)) {
				const res = path_1.default
					.join(path_1.default.dirname(module), _path)
					.replace(/\\/g, "/");
				return prefix + res;
			}
			return prefix + _path;
...
function removeAllTSBoilerplate(source) {
	const seen = new Array(BOILERPLATE.length).fill(
		true,
		0,
		BOILERPLATE.length,
	);
