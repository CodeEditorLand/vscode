...
import { getL10nFilesFromXlf, getL10nJson, getL10nXlf, l10nJsonDetails, l10nJsonFormat, } from "@vscode/l10n-dev";
...
export class XLF {
    ...
    public toString(): string {
        this.appendHeader();
        for (const file of Object.keys(this.files).sort()) {
            this.appendNewLine(`<file original="${file}" source-language="en" datatype="plaintext"><body>`, 2);
            for (const item of this.files[file].sort((a: Item, b: Item) => {
    ...
    public addFile(original: string, keys: (string | LocalizeInfo)[], messages: string[]) {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            this.files[original].push({
                id: realKey,
                message: encodeEntities(messages[i]),
                comment: comment,
            });
        }
    }
    ...
    static parse = function (xlfString: string): Promise<l10nJsonDetails[]> {
        return new Promise((resolve, reject) => {
            ...
            new xml2js.Parser().parseString(xlfString, function (err: any, result: any) {
                if (err) {
        ...
        if (path.basename(file.path)
            ...
            try {
                ...
                if (NLSKeysFormat.is(json)) {
                    ((base: string, fileHeader: string, languages: Language[], json: NLSKeysFormat, emitter: ThroughStream) => {
                        ...
                        ((languages: Language[]): Language[] => {
                            ...
                            const i18nFile = path.join(languageDirectory, `vscode-language-pack-${(language.translationId || language.id)}`, "translations", "main.i18n.json");
                            let allMessages: I18nFormat | undefined;
                            if (fs.existsSync(i18nFile)) {
                                allMessages = JSON.parse(((content: string): string => {
                                    return content.replace(/("[^"\\]*(?:\\.[^"\\]*)*")|('[^'\\]*(?:\\.[^'\\]*)*')|(\/\*[^\/\*]*(?:(?:\*|\/)[^\/\*]*)*?\*\/)|(\/{2,}.*?(?:(?:\r?\n)|$))|(,\s*[}\]])/g, (match, _m1: string, _m2: string, m3: string, m4: string, m5: string) => {
                            ...
                            for (const [moduleId, nlsKeys] of json) {
                                for (const nlsKey of nlsKeys) {
                                    nlsResult.push(allMessages?.contents[moduleId]?.[nlsKey]); // pushing `undefined` is fine, as we keep english strings as fallback for monaco editor in the build
                                    0++;
                                }
                            }
...
globalThis._VSCODE_NLS_LANGUAGE=${JSON.stringify(language.id)};`),
                    ...
                    })(file.base, opts.fileHeader, opts.languages, json, this);
                }
            }
            catch (error) {
...
export function getResource(sourceFile: string): Resource {
    ...
    if (/^vs\/platform/.test(sourceFile)) {
        ...
        return { name: "vs/server", project: "vscode-server" };
    }
            ...
            if (file.isBuffer()) {
                ...
                for (const coreModule in json.keys) {
                    ...
                    if (keys.length !== messages.length) {
                        ...
                        if (!xlf) {
                            xlf = new XLF(projectResource.project);
                            xlfs[resource] = xlf;
                        }
                ...
                for (const resource in xlfs) {
                    ...
                    this.queue(new File({
                        path: `${xlf.project}/${resource.replace(/\//g, "_")}.xlf`,
...
export function createXlfFilesForExtensions(): ThroughStream {
    ...
    return through(function (this: ThroughStream, extensionFolder: File) {
        ...
        if (!fs.statSync(extensionFolder.path).isDirectory()) {
            return;
        }
        ...
        const manifestJson = JSON.parse(fs.readFileSync(path.join(extensionFolder.path, "package.json"), "utf-8"));
        ...
        ], { allowEmpty: true }), ((extensionFolderName: string, prefixWithBuildFolder: boolean): NodeJS.ReadWriteStream => {
        ...
        })(extensionFolderName, EXTERNAL_EXTENSIONS.includes(extensionId))).pipe(through(function (file: File) {
            if (file.isBuffer()) {
                const buffer: Buffer = file.contents as Buffer;
                const basename = path.basename(file.path);
                if (basename === "package.nls.json") {
                    getL10nMap().set(`extensions/${extensionId}/package`, JSON.parse(buffer.toString("utf8")));
                }
                    ...
                    for (const file in json) {
                        ...
                        for (let i = 0; i < fileContent.messages.length; i++) {
                            ...
                            info[key] = comment
                                ? { message, comment }
                                :
                                    fileContent.messages[i];
                        }
                        getL10nMap().set(`extensions/${extensionId}/${path.relative(`.build/extensions/${extensionFolderName}`, path.dirname(file.path))}/${file}`, info);
                    }
                }
                else if (basename === "bundle.l10n.json") {
                    getL10nMap().set(`extensions/${extensionId}/bundle`, JSON.parse(buffer.toString("utf8")));
                }
                else {
...
export function createXlfFilesForIsl(): ThroughStream {
    return through(function (this: ThroughStream, file: File) {
        ...
        new TextModel(file.contents.toString()).lines.forEach((line) => {
            if (line.length === 0) {
                return;
            }
        ...
        xlf.addFile(file.path
            .substring(file.cwd.length + 1, file.path.split(".")[0].length)
            .replace(/\\/g, "/"), keys, messages);
        this.queue(new File({
            path: path.join(projectName, resourceFile),
...
export function prepareI18nPackFiles(resultingTranslationPaths: TranslationPath[]): NodeJS.ReadWriteStream {
    ...
    return through(function (this: ThroughStream, xlf: File) {
        ...
        const parsePromise = getL10nFilesFromXlf(xlf.contents.toString());
        ...
        parsePromise
            ...
            resolvedFiles.forEach((file) => {
                ...
                if (project === extensionsProject) {
                    ...
                    extPack.contents[path.substring(path.indexOf("/", firstSlash + 1)
                        + 1)] =
        ...
        Promise.all(parsePromises)
            ...
            this.queue(createI18nFile("./main", mainPack));
            for (const extensionId in extensionsPacks) {
                this.queue(createI18nFile(`extensions/${extensionId}`, extensionsPacks[extensionId]));
                resultingTranslationPaths.push({
                    id: extensionId,
...
export function prepareIslFiles(language: Language, innoSetupConfig: InnoSetup): ThroughStream {
    ...
    return through(function (this: ThroughStream, xlf: File) {
        ...
        parsePromise
            .then((resolvedFiles) => {
            resolvedFiles.forEach((file) => {
                this.queue(createIslFile(file.name, file.messages, language, innoSetupConfig));
            });
        })
            .catch((reason) => {
            this.emit("error", reason);
...
function createIslFile(name: string, messages: l10nJsonFormat, language: Language, innoSetup: InnoSetup): File {
    ...
    return new File({
        path: `${path.basename(name)}.${language.id}.isl`,
        contents: Buffer.from(iconv.encode(Buffer.from(content.join("\r\n"), "utf8").toString(), innoSetup.codePage)),
    });
}
