...
function createESMSourcesAndResources2(options) {
	...
	const OUT_RESOURCES_FOLDER = path_1.default.join(
		REPO_ROOT,
	...
	for (const file of allFiles) {
		...
		if (file === "tsconfig.json") {
			const tsConfig = JSON.parse(
				fs_1.default
					.readFileSync(path_1.default.join(SRC_FOLDER, file))
					.toString(),
			);
			tsConfig.compilerOptions.module = "es2022";
			tsConfig.compilerOptions.outDir = path_1.default
				.join(
					path_1.default.relative(OUT_FOLDER, OUT_RESOURCES_FOLDER),
					"vs",
				)
				.replace(/\\/g, "/");
			write(
		...
		if (
			...
			write(
				...
				fs_1.default.readFileSync(
					path_1.default.join(SRC_FOLDER, file),
				),
			);
			continue;
		}
