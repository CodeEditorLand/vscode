...
function getMassagedTopLevelDeclarationText(ts: typeof import("typescript"), sourceFile: ts.SourceFile, declaration: TSTopLevelDeclare, importName: string, usage: string[], enums: IEnumEntry[]): string {
    ...
    if (declaration.kind === ts.SyntaxKind.InterfaceDeclaration ||
        ...
        const staticTypeName = ((ts: typeof import("typescript"), declaration: ts.InterfaceDeclaration | ts.ClassDeclaration): boolean => {
            ...
            try {
                ...
                if (memberText.indexOf("@internal") >= 0 ||
                    ...
                    const memberAccess = memberName.indexOf(".") >= 0
                        ? `['${memberName}']`
                        : `.${memberName}`;
                    if (((ts: typeof import("typescript"), member: ts.ClassElement | ts.TypeElement): boolean => {
...
function format(ts: typeof import("typescript"), text: string, endl: string): string {
    ...
    return applyEdits(text, (<any>ts).formatting.formatDocument(ts.createSourceFile("file.ts", text, ts.ScriptTarget.Latest, 
    /*setParentPointers*/ true), getRuleProvider(tsfmt), tsfmt));
...
function createReplacer(data: string): (str: string) => string {
    ...
    data.split(";").forEach((rawDirective) => {
        ...
        findStr = "\\b" + findStr + "\\b";
        directives.push([new RegExp(findStr, "g"), pieces[1]]);
    });
    return ((directives: [
        RegExp,
        string
...
export class DeclarationResolver {
    ...
    public getDeclarationSourceFile(moduleId: string): ts.SourceFile | null {
        if (this._sourceFileCache[moduleId]) {
            if (this._sourceFileCache[moduleId]!.mtime !==
                this._fsProvider.statSync(this._getFileName(moduleId)).mtime.getTime()) {
                this._sourceFileCache[moduleId] = null;
            }
        }
        if (!this._sourceFileCache[moduleId]) {
    ...
    private _getDeclarationSourceFile(moduleId: string): CacheEntry | null {
        ...
        if (/\.d\.ts$/.test(moduleId)) {
            return new CacheEntry(this.ts.createSourceFile(fileName, this._fsProvider
                .readFileSync(moduleId, fileName)
                .toString(), this.ts.ScriptTarget.ES5), mtime);
        }
        return new CacheEntry(this.ts.createSourceFile(fileName, this.ts.createLanguageService(new TypeScriptLanguageServiceHost(this.ts, {}, {
            "file.ts": this._fsProvider
                .readFileSync(moduleId, fileName)
                .toString(),
        }, {})).getEmitOutput("file.ts", true, true).outputFiles[0]
            .text, this.ts.ScriptTarget.ES5), mtime);
    }
}
export function run3(resolver: DeclarationResolver): IMonacoDeclarationResult | null {
    return ((ts: typeof import("typescript"), sourceFileGetter: SourceFileGetter): IMonacoDeclarationResult | null => {
        const t = ((ts: typeof import("typescript"), recipe: string, sourceFileGetter: SourceFileGetter): ITempResult | null => {
            const endl = /\r\n/.test(recipe) ? "\r\n" : "\n";
            const result: string[] = [];
            ...
            const generateUsageImport = (moduleId: string) => {
                const importName = "m" + ++0;
                usageImports.push(`import * as ${importName} from './${moduleId.replace(/\.d\.ts$/, "")}';`);
                return importName;
            };
            const enums: IEnumEntry[] = [];
            let version: string | null = null;
            recipe.split(endl).forEach((line) => {
                if (failed) {
                    return;
                }
                ...
                if (m1) {
                    ...
                    m1[3].split(/,/).forEach((typeName) => {
                        typeName = typeName.trim();
                        if (typeName.length === 0) {
                            return;
                        }
                        const declaration = ((ts: typeof import("typescript"), sourceFile: ts.SourceFile, typeName: string): TSTopLevelDeclare | null => {
                        ...
                        result.push(createReplacer(m1[2])(getMassagedTopLevelDeclarationText(ts, sourceFile, declaration, generateUsageImport(moduleId), usage, enums)));
                    });
                    return;
                }
                ...
                if (m2) {
                    ...
                    ((ts: typeof import("typescript"), sourceFile: ts.SourceFile): TSTopLevelDeclare[] => {
                        ...
                        visitTopLevelDeclarations(ts, sourceFile, (node) => {
                            if (node.kind === ts.SyntaxKind.InterfaceDeclaration ||
                                ...
                                if (getNodeText(sourceFile, {
                                    pos: interfaceDeclaration.pos,
                                    end: interfaceDeclaration.name.pos,
                                }).indexOf("@internal") === -1) {
                                    all.push(node);
                                }
                            }
                            else {
                                if (getNodeText(sourceFile, node).indexOf("@internal") === -1) {
                                    all.push(node);
                                }
                            }
                            return false /*continue*/;
                        ...
                        return all;
                    })(ts, sourceFile).forEach((declaration) => {
                        if (isDeclaration(ts, declaration) && declaration.name) {
                            ...
                            for (let i = 0; i < typesToExcludeArr.length; i++) {
                                if (getNodeText(sourceFile, declaration).indexOf(typesToExcludeArr[i]) >= 0) {
                                    return;
                                }
                            }
                        }
                        result.push(createReplacer(m2[2])(getMassagedTopLevelDeclarationText(ts, sourceFile, declaration, generateUsageImport(moduleId), usage, enums)));
                    });
                    return;
                }
                result.push(line);
        ...
        })(ts, fs.readFileSync(path.join(__dirname, "../monaco/monaco.d.ts.recipe")).toString(), sourceFileGetter);
        if (!t) {
            return null;
        }
        const result = t.result;
        const isTheSame = fs.readFileSync(DECLARATION_PATH).toString().replace(/\r\n/gm, "\n")
            ===
                result.replace(/\r\n/gm, "\n");
        return {
            content: result,
            usageContent: t.usageContent,
            enums: t.enums,
            filePath: DECLARATION_PATH,
            isTheSame,
        };
    })(resolver.ts, (moduleId: string) => resolver.getDeclarationSourceFile(moduleId));
}
interface ILibMap {
