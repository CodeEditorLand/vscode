            ...
            edit: (parsedJson, file) => {
                ...
                switch (file.basename) {
                    ...
                    case "nls.metadata.json": {
                        ...
                        for (const module of Object.keys(parsedJson)) {
                ...
                const manifestJson = JSON.parse(readFileSync(path.join(path.join(file.base, file.relative.split("/")[0]), "package.json"), "utf-8"));
                return { [(manifestJson.publisher + "." + manifestJson.name)]: parsedJson };
            },
        }));
        es.merge(combinedMetadataJson, vfs.src("out-build/nls.messages.js", {
            base: "out-build",
        }))
