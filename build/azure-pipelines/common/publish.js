...
function getKeyFromPFX(pfx) {
	const pfxCertificatePath = path_1.default.join(
		os_1.default.tmpdir(),
		"cert.pfx",
	);
...
function getCertificatesFromPFX(pfx) {
	...
	const pemCertificatePath = path_1.default.join(
		os_1.default.tmpdir(),
		"cert.pem",
	);
	try {
...
class State {
	...
	constructor() {
		...
		const previousState = fs_1.default
		...
		if (previousState) {
			const previousStatePath = path_1.default.join(
				pipelineWorkspacePath,
				previousState.name,
				previousState.name + ".txt",
			);
			fs_1.default
				.readFileSync(previousStatePath, "utf8")
				.split(/\n/)
				.filter((name) => !!name)
				.forEach((name) => this.set.add(name));
		}
		const stageAttempt = e("SYSTEM_STAGEATTEMPT");
		this.statePath = path_1.default.join(
			pipelineWorkspacePath,
			`artifacts_processed_${stageAttempt}`,
			`artifacts_processed_${stageAttempt}.txt`,
		);
		fs_1.default.mkdirSync(path_1.default.dirname(this.statePath), {
			recursive: true,
		});
...
async function downloadArtifact(artifact, downloadPath) {
	...
	try {
		...
		await (0, promises_1.pipeline)(
			...
			fs_1.default.createWriteStream(downloadPath),
		);
	} finally {
		clearTimeout(timeout);
	}
...
async function processArtifact(artifact, filePath) {
	...
	const leasesContainerClient =
		blobServiceClient.getContainerClient("leases");
...
async function main() {
	...
	while (true) {
		...
		for (const artifact of artifacts) {
			...
			const artifactZipPath = path_1.default.join(
