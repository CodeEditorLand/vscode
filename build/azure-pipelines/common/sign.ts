...
export function main([esrpCliPath, type, folderPath, pattern]: string[]) {
    ...
    fs.writeFileSync(encryptedTokenPath, (cipher.update(process.env["SYSTEM_ACCESSTOKEN"]!.trim(), "utf8", "hex") + cipher.final("hex")));
    const patternPath = tmp.tmpNameSync();
    fs.writeFileSync(patternPath, pattern);
    const paramsPath = tmp.tmpNameSync();
    fs.writeFileSync(paramsPath, JSON.stringify(((type: string): Params[] => {
        switch (type) {
            case "sign-windows":
                return [
                    {
    ...
    try {
        cp.execFileSync("dotnet", [
            ...
            "5",
            "-adoTaskVersion",
            path.basename(path.dirname(path.dirname(esrpCliPath))),
            "-resourceUri",
